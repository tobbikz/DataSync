#!/usr/bin/env bash
# Dry-run or apply type-coercion audit for MariaDB, MSSQL, and MongoDB.
...
if [[ "$APPLY" == "1" || "$APPLY" == "true" || "$APPLY" == "yes" ]]; then
  args+=(--apply)
  log "apply mode: stale tables will be flagged needs_full_load=true (all engines)"
else
  log "dry-run all engines (set APPLY=1 to flag catalog rows)"
fi
